...
export async function processCapture(dataUrl: string): Promise<{ image: string; hash: string }> {
  ...
  try {
    ...
    if (!jpegCtx) throw new Error("2d context unavailable");
    ...
    if (!hashCtx) throw new Error("2d context unavailable");
