...
type CapturePayload = import("./messages").CapturePayload;
type BackgroundToContent = import("./messages").BackgroundToContent;
type ContentToBackground = import("./messages").ContentToBackground;

chrome.runtime.onMessage.addListener((msg: BackgroundToContent, _sender, sendResponse) => {
  if (msg.kind !== "capture-request") return false;
  ...
  const out: ContentToBackground = { kind: "capture", payload };
  sendResponse(out);
